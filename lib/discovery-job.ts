...
import {isSalesAndTrading,canonicalCompany,dailyCompanyQuotas,TARGET_COMPANIES} from './targeting.ts';
import {identitiesFor} from './safety.ts';
import {queryPlan,queryWithPreferences,matchesPreferences,companyCount,emptyObservation,publishedPeople,corroborates,discoveredContact,type QueryObservation,type DiscoveryPreferences} from './discovery.ts';
import {allowedSource,PublicSourceReader,DISCOVERY_AGENT} from './discovery-policy.ts';

type Environment=Record<string,string|undefined>;
export type DiscoveryConfig={enabled:boolean;configured:boolean;apiKey:string;hosts:string[];agent:string;dailyCap:50;companyCap:4;searchCap:number;pageCap:number;note:string};
...
export function discoveryConfig(env:Environment=process.env):DiscoveryConfig {
  ...
  return {enabled:env.DISCOVERY_ENABLED==='true',configured,apiKey:env.BRAVE_SEARCH_API_KEY||'',hosts,agent:`${DISCOVERY_AGENT}/1.0 (+${contact})`,dailyCap:50,companyCap:4,searchCap:bounded(env.DISCOVERY_MAX_SEARCHES,120,200),pageCap:bounded(env.DISCOVERY_MAX_PAGES,200,400),note:configured?'Configured for a 50-contact daily Sales & Trading target across the 15-company allowlist; published email still requires outreach review.':'Configure a licensed Brave Search API key, approved public source hosts and a bot contact URL to activate discovery.'};
}
type JobPayload={target:number;startedAt:string;finishedAt?:string;updatedAt:string;searchRequests:number;pageRequests:number;completedQueries:string[];queryStats:QueryObservation[];accepted:number;salesAndTrading:number;fallback:number;companyCounts:Record<string,number>;companyCap?:number;note:string;error?:string;plan?:string[]};
function initialPayload(at:string):JobPayload{return {target:50,startedAt:at,updatedAt:at,searchRequests:0,pageRequests:0,completedQueries:[],queryStats:[],accepted:0,salesAndTrading:0,fallback:0,companyCounts:{},note:'Public-source discovery; no messages created or sent.'};}
export function payloadSummary(payload:Partial<JobPayload>){const target=payload.target||50,accepted=payload.accepted||0;return {target,shortfall:Math.max(0,target-accepted),accepted,salesAndTrading:payload.salesAndTrading||0,fallback:payload.fallback||0,requests:(payload.searchRequests||0)+(payload.pageRequests||0),rejected:(payload.queryStats||[]).reduce((n,q)=>n+q.rejected,0)};}
...
export async function runDiscoveryJob(db:SupabaseClient,owner:string,env:Environment=process.env,io:DiscoveryIO={}) {
  ...
  const deadline=Date.now()+25*60_000;const lockUntil=new Date(Date.now()+30*60_000).toISOString();
  ...
  if(claim.error){
    ...
    if(['complete','cancelled'].includes(old.data.state)||old.data.attempts>=6||Date.parse(old.data.locked_until||'')>Date.now())return {ok:true,duplicate:true,state:old.data.state};
  ...
  payload.companyCap=config.companyCap;
  const quotaUsed=()=>Math.max(payload.accepted,Object.values(payload.companyCounts).reduce((sum,count)=>sum+count,0));
  ...
  try{
    ...
    const companyHistory=new Map<string,number>();
    for(const row of foundToday){const company=canonicalCompany(row.data.discoveryCompany||row.data.company);companyHistory.set(company,(companyHistory.get(company)||0)+1);}
    for(const [company,count] of Object.entries(payload.companyCounts))companyHistory.set(company,Math.max(companyHistory.get(company)||0,count));
    payload.companyCounts=Object.fromEntries(companyHistory);
    if(payload.accepted>Object.values(payload.companyCounts).reduce((sum,count)=>sum+count,0)&&payload.accepted<config.dailyCap)throw Error('Daily company history is incomplete; discovery held until the next UTC day');
    ...
    const recipes=queryPlan(observations,day),quotas=dailyCompanyQuotas(day),dayIndex=Math.floor(Date.parse(day+'T00:00:00Z')/86400000);
    const companies=[...TARGET_COMPANIES.slice(dayIndex%TARGET_COMPANIES.length),...TARGET_COMPANIES.slice(0,dayIndex%TARGET_COMPANIES.length)];
    const planned=recipes.flatMap((recipe,recipeIndex)=>companies.map((company,companyIndex)=>({id:`${company.key}:${recipe.id}`,recipe,company,recipeIndex,companyIndex})));
    payload.plan=planned.map(item=>item.id);await persist();
    for(const item of planned){
      const {recipe,company,recipeIndex,companyIndex}=item;
      if(quotaUsed()>=config.dailyCap)break;
      if(payload.completedQueries.includes(item.id)||companyCount(payload.companyCounts,company.key)>=quotas[company.key])continue;
      const stats=payload.queryStats.find(q=>q.id===item.id)||emptyObservation(item.id);
      ...
      const urls=await search(`${queryWithPreferences(recipe.query,preferences,day,{recipeIndex:recipeIndex*TARGET_COMPANIES.length+companyIndex,company:company.label})} (${config.hosts.map(h=>'site:'+h).join(' OR ')})`);stats.results+=urls.length;
      for(const url of urls){
        ...
        for(const person of people){
          ...
          if(!isSalesAndTrading({title:person.title,industry:'Sales & Trading'})){stats.rejected++;continue;}
          const personCompany=canonicalCompany(person.company);
          if(personCompany!==company.key){stats.rejected++;continue;}
          if(companyCount(payload.companyCounts,personCompany)>=quotas[personCompany]){stats.rejected++;stats.companyCapped=(stats.companyCapped||0)+1;continue;}
          ...
          // Reserve the company slot BEFORE insertion. An uncertain crash consumes
          // the slot, including when the resulting contact is later deleted.
          const before=companyCount(payload.companyCounts,personCompany);
          payload.companyCounts={...payload.companyCounts,[personCompany]:before+1};await persist();
          ...
          if(!result.data){stats.duplicates++;payload.companyCounts={...payload.companyCounts,[personCompany]:before};await persist();continue;}
          identitiesFor(contact).forEach(k=>identities.add(k));stats.accepted++;payload.accepted++;
          payload.salesAndTrading++;
          await persist();
        }
      }
      payload.completedQueries.push(item.id);await persist();
    }
    payload.finishedAt=new Date().toISOString();payload.error=undefined;
    const shortfall=config.dailyCap-quotaUsed();
    payload.note=shortfall===0?'Completed the 50-contact Sales & Trading batch across the approved company quotas. Contacts retain public evidence and require outreach review.':`Daily batch shortfall: ${shortfall}. No unverified or off-list contacts were substituted.`;
    if(shortfall>0)payload.error=payload.note;
    await persist(shortfall===0?'complete':'partial');
    return {ok:shortfall===0,state:shortfall===0?'complete':'partial',shortfall,summary:payloadSummary(payload)};
  }catch(error){

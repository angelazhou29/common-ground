...
6. Dispatch **Daily public-source contact discovery** once, then inspect its state/evidence/counts in the app. It schedules every day at 13:23 UTC, subject to GitHub delays. It executes `jobs/discover.ts` directly against Supabase and Brave, preserving Vercel protection. Do not create deployment-protection exceptions or share/bypass links for scheduling. `POST /api/jobs?kind=discovery` also supports an already-authorized protected caller with the server cron secret; Actions does not use that endpoint.

The target and maximum are 75 new contacts daily, including weekends. Only the 15 owner-approved employers qualify, using canonical employer names rather than name/title substrings. Unfilled targets are reported as shortfalls. Default budgets remain 12 search requests (including corroboration), and 24 page requests (including robots files) per UTC day. Configurable maxima are 75 contacts, 30 searches, and 60 page requests. These budgets can prevent reaching the target; no paid quota has been activated or increased. Source spacing is at least 1.1 seconds or its longer crawl delay, including before the first biography. Delays above 30 seconds are skipped. Jobs stop after roughly four minutes. Zero qualifying contacts is a valid result; no fictional contacts fill a shortfall.

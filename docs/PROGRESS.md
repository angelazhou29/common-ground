...
Only the 15 companies listed in lib/company-scope.ts qualify; explicit company aliases are normalized, with no name/title substring qualification. Balyasny added; RBC, CIBC and Ares removed. Target/max discovery is 75 NEW people every calendar day, including weekends. Search/page budgets remain unchanged and unfilled targets produce a reported shortfall, never fabricated or off-scope replacements. Existing records remain in history. Company settings saved to Supabase. 50 application tests and TypeScript passed; migration/provider activation still pending.

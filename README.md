...
The Contacts page also has a strict human-in-the-loop LinkedIn intake. Download its evidence CSV, manually confirm each direct profile and current role, then enrich only the reviewed rows. `pnpm hitl:enrich -- input.csv verified.csv` optionally uses Hunter's found-only endpoint and a separate fresh verification call. It accepts at most five valid contacts per target company for a balanced 75-person pool and rejects inferred, accept-all, stale, off-list, non-Sales-and-Trading, and malformed results. Importing the verified output runs the database lifetime-identity check atomically. See [HITL intake](docs/HITL_INTAKE.md).
